...
document.getElementById("my-form").addEventListener("submit", function (event) {
  ...
  const status = document.getElementById("status").value; // Including status

  // Display the comment immediately on the page
  ...
  // Send the comment to Google Sheets
  fetch('https://script.google.com/macros/s/AKfycbzEFBd20ioD6pnNZnLjuUUvLAAtsxYy5v0oBjjz6V76lliIYewy-nbyP2bLXDuC0z6X/exec', {
    method: 'POST',
...
// Fetch and load comments from Google Sheets
const SHEET_ID = '1s2R3OQpoYyHr-h2BKFzrkmr9WlkVcWXiOhDekxzN9_8';  // Replace with your Google Sheet ID
const API_KEY = 'AIzaSyAWJGx20rtHsJDC0bjc8L356wbfBKCvdy8';  // Replace with your Google Sheets API Key

async function loadComments() {
  const url = `https://sheets.googleapis.com/v4/spreadsheets/${SHEET_ID}/values/Ucapan&Status!A2:C?key=${API_KEY}`;
  ...
  data.values.forEach(row => {
    const commentElement = document.createElement("div");
    commentElement.className = "comment-box";
    commentElement.innerHTML = `<strong>${row[1]}</strong>: ${row[2]}<br><small>${row[3]}</small>`; // Adjust to match your columns
